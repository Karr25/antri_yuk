<?php 
	...
	if($extTypeRes != 'image' || $_FILES['file_foto']['size'] > 10000000)
	{
		...
		header('Location:http://localhost/antriyuk/admin_side/false_site/error_upload_foto.php');
		...
		move_uploaded_file($_FILES['file_foto']['tmp_name'], $_SERVER['DOCUMENT_ROOT'] . '/antriyuk/node_module/assets/foto_profil/'.$nama_file_baru);


		$queri = mysqli_query($conn, "UPDATE tbl_admin SET file_foto_profil = '".$nama_file_baru."' WHERE id_admin = '".$_SESSION['user']['id_admin']."' ");

		unlink($_SERVER['DOCUMENT_ROOT'] . '/antriyuk/node_module/assets/foto_profil/'.$_SESSION['user']['file_foto_profil']);
